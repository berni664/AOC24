...
mpz_class digits(mpz_class input);
mpz_class concatenate(const mpz_class& a, const mpz_class& b);
   ...
   for(const auto& perm : perms){
      ...
      for(size_t i{0}; i < perm.length(); ++i){
         ...
         if(perm[i] == '+')
            current += input.at(j);
         if(perm[i] == '&')
            current = concatenate(current, input.at(j));
         j++; 
         if(current > target)
            break;
      }
...
mpz_class concatenate(const mpz_class& a, const mpz_class& b) {
    ...
    std::string B = b.get_str();
    std::string concatenatedStr = A + B;
    return mpz_class(concatenatedStr);
}
